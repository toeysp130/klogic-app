...
export default function Profile() {
  ...
  const [moomooData , setMoomooData] = useState(userData.subject.filter(item => item.id_sub === sid)[0]);
  ...
  const moomooSubmit = e => {
    ...
    const tempMoomoo = {...userData , subject:userData.subject.filter(item => item.id_sub !== moomooData.id_sub)}
    try{
  ...
  return (
    <>
      ...
      component="form"
      sx={{
        '& .MuiTextField-root': { m: 5, mt: 15, width: '20ch' },textAlign: "center",
      }}
      noValidate
      autoComplete="off"
    >
      ...
      <Box sx={{ "& button": { m: 1 } }}>
            <div>
              <Button size="large" variant="contained" color="error" type="button" onClick={moomooSubmit}> 
                WITHDRAW
              </Button>
            </div>
          </Box>
