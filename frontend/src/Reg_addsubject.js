...
export default function Profile() {
  ...
  const [subjectData, setSubjectData] = useState({ id_sub: "", section: "" });

  const { id_sub, section } = subjectData;
  ...
  const mamaa = e => {
    e.preventDefault();
    try{
        axios.post(`http://192.168.1.2:9999/api/subject/find/${id_sub}`)
        .then(result=> {
          if(result.data){
            axios.post(`http://192.168.1.2:9999/api/users/chk/subject/${currentUser._id}/${id_sub}`)
            .then((res)=>{
              if(res.data.length === 0){
                window.location.href = process.env.PUBLIC_URL + '/cfadd'
                localStorage.setItem('subject' , JSON.stringify(result.data));
              }
              else {
